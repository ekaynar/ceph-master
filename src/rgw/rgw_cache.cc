...
//    get_obj_data *d = (get_obj_data *)req->op_data;
   ldout(cct, 10) << __func__  << " range " << range << dendl;
    ...
    std::string AWSAccessKeyId=req->c_obj->accesskey.id;
    std::string YourSecretAccessKeyID=req->c_obj->accesskey.key;
   ldout(cct, 10) << __func__  << " AWSAccessKeyId  "<<  AWSAccessKeyId << dendl;
   ldout(cct, 10) << __func__  << " YourSecretAccessKeyID" <<YourSecretAccessKeyID << dendl;
    ...
    std::string  loc = req->c_obj->destination + uri;
    ...
    if(curl_handle) {
        ...
        ldout(cct,10) << "Ugur: curl_easy_perform() failed " << curl_easy_strerror(res) << " oid " << req->key << dendl;
        return -1;
    }else{return 0;}
