...
void DataCache::incr_writeback(uint64_t len){
	cache_lock.lock();
	writeback_hit++;
	writeback_hit_bytes += len;
	ldout(cct, 1) << " writeback_hit++ " << writeback_hit<< dendl;
    ldout(cct, 1) << " writeback_hit_bytes " << writeback_hit_bytes<< dendl;
	cache_lock.unlock();
	}
...
void DataCache::copy_aged_obj(RGWRados *store, uint64_t interval){
	...
	ldout(cct, 1) << __func__ << " in " << free_wb_cache_capacity <<dendl;	
	...
	int64_t free_ratio = (int64_t)(cct->_conf->rgw_wb_cache_size * 0.10);
...
//	  if (free_wb_cache_capacity > free_ratio){
	  if (seconds < (double(interval)*60) )
		break;
//	  } else{
//	   ldout(cct, 1) << __func__ <<  " " << free_wb_cache_capacity <<dendl;	
//	  }
	...
	//small object coalesing writes
	 } else if (c_obj->size_in_bytes < cct->_conf->rgw_obj_stripe_size && cct->_conf->enable_coalesing_write){
		string obj_id = del_entry->obj_id;
		obj_cache_lock.lock();
...
void DataCache::directoryUpdate(string del_oid, int64_t del_weight, bool t=true)
{
  ldout(cct, 20) << __func__ << dendl;
...
int cacheAioWriteRequest::create_io(bufferlist& bl, uint64_t len, string key) {
  ...
  std::string location = cct->_conf->rgw_datacache_path + "/"+ tmp;
...
/*
    ...
    --m_open_list_end;
    auto key_position = m_key_map.emplace(el.obj_id, m_open_list_end).first;
...
*/
  ...
  if (cct->_conf->rgw_local_lru == false) { 
...
int DataCache::deleteFromWriteObj(cache_obj *c_obj){
  ...
  cache_lock.lock();
	 if (lfu_cache_map.find(key) != lfu_cache_map.end()) {
        ...
        if (!isRemote){
          ...
          ldout(cct, 0) << __func__ << "local request:"<< key  << "new weight" << new_freq <<dendl;
...
//		  avg_w = round(total_cache_weight/m_dynamic_age_list.size());
			...
			local_remote_hit +=1;
			ldout(cct, 0) << __func__ << " remote_cache_request_hit: "<< local_remote_hit << dendl;
			blkDirectory->updateGlobalWeight(key, cache_weight, false);
			ldout(cct, 0) << __func__ << " remote req hit cache weight"<< cache_weight << " oid "<< key << dendl;
	...
	if (isRemote){
	  remote_miss ++;
	   ldout(cct, 0) << __func__ << " remote_request_miss: "<< remote_miss << dendl;	
	}
	auto end2 = chrono::steady_clock::now();
  ldout(cct,10) << __func__  << " lfuda miss ms " << chrono::duration_cast<chrono::microseconds>(end2 - start).count()  << dendl;
  }
  cache_lock.unlock();
  return exist;
  ...
  const char x = '/';
  const char y = '_';
  std::replace(tmp_oid.begin(), tmp_oid.end(), x, y);
  string location = cct->_conf->rgw_datacache_path + "/"+ tmp_oid;
  ldout(cct, 0) << __func__ << "key:"<< key << " location: "<< location << dendl;
  cache_lock.lock();
  ...
  cache_lock.unlock();
  auto end2 = chrono::steady_clock::now();
  ldout(cct,10) << __func__ << " lru req ms " << chrono::duration_cast<chrono::microseconds>(end2 - start).count()  << dendl;
  return exist;
  }
}
...
size_t DataCache::lfuda_eviction2(){
  ...
  while(!find){
	  //if ( freq_map.find(min_freq) != freq_map.end() && freq_map[min_freq].size() != 0) {
	  if ( freq_map.count(min_freq) != 0 && !freq_map[min_freq].empty()) {
		find = true;
		del_weight = min_freq;
	    ldout(cct, 10) << __func__  << "victim del_weight: "<< del_weight << dendl;
		ldout(cct, 10) << __func__  <<  " mapsize " <<freq_map[min_freq].size() << dendl;
		ldout(cct, 10) << __func__  <<  " mapsize " <<freq_map.count(min_freq)<< dendl;
		del_oid = freq_map[min_freq].front();
	    ldout(cct, 10) << __func__  << "victim del_oid: "<< del_oid << dendl;
  ...
  ldout(cct,10) << __func__  << " before ms " << chrono::duration_cast<chrono::microseconds>(end3 - start).count() << dendl;
  ...
  if (victim->cachedOnRemote && ret>=0) {
    ...
    return freed_size;
        ...
        if (victim->access_count != 0) {
          ldout(cct, 10) << __func__  <<" last copy with global access: " << del_oid << " new weight: "<< del_weight << dendl;
          cache_lock.lock();
          del_weight += victim->access_count;
          element el;
		  el.obj_id = del_oid;
		  el.size_in_bytes = tmp.size_in_bytes;
		  lfu_cache_map[del_oid] = {el,del_weight} ;
		  freq_map[del_weight].push_back(del_oid);
		  key_iter[del_oid] = --freq_map[del_weight].end(); 
		  total_cache_weight += victim->access_count;
          cache_lock.unlock();
          auto end2 = chrono::steady_clock::now();
          ldout(cct,10) << __func__  << " last copy, dw ms " << chrono::duration_cast<chrono::microseconds>(end2 - start).count()  << dendl;
 ...
 */
  ...
  ldout(cct, 10) << __func__ << "key: "<<obj_id<< "len: "<< len << "free_data_cache_size: "<<free_data_cache_size << "_:"<< free_data_cache_size << "outstanding_write_size  "<< outstanding_write_size << "_ " << _outstanding_write_size << dendl;
//  while (len >= (_free_data_cache_size - _outstanding_write_size + freed_size)){
   while (static_cast<int64_t>(len) >= static_cast<int64_t>(_free_data_cache_size - _outstanding_write_size + freed_size)){
    ldout(cct, 10) << "Datacache Eviction r=" << ret << "key"<<obj_id <<dendl;
	if(cct->_conf->rgw_lfuda == true)
	  ret = lfuda_eviction2();
	else
	  ret = lru_eviction();
